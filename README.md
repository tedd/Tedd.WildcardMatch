...
The framework utilizes deterministic lexical transpilation to convert wildcard patterns into compiled Regular Expressions. The execution flow strictly enforces full-string matching:

1. **Lexical Transpilation:** The input wildcard string is escaped to neutralize pre-existing Regex control characters.
2. **Token Replacement:** Wildcard tokens are substituted with their Regex equivalents (`*` becomes `.*`, `?` becomes `.`).
3. **Boundary Anchoring:** The entire pattern is anchored using `^` (start of string) and `$` (end of string). **Note:** This mathematically enforces a full-string match. Substring matches are not supported unless explicitly declared via `*` prefixes and suffixes.
4. **Regex Execution:** The transpiled pattern is evaluated by the .NET Regular Expression engine.

### Established Framework Capabilities
The core operational reality is built upon deterministic token replacement followed by full-string boundary-anchored Regex execution. The execution flow does not rely on speculative evaluation or substring heuristics. It explicitly guarantees exact end-to-end matching (except when users manually specify wildcard prefixes/suffixes).

### Hypotheses (Planned Future Enhancements)
We hypothesize the introduction of a modernized, high-performance hierarchical data binding model alongside a routed event infrastructure. These enhancements would aim to offload match evaluation overhead and distribute state updates across deeply nested architectural components, although these mechanics are strictly conceptual and not present in the contemporary framework.
...
WildcardMatch wmc = new("*or*ips?m*", WildcardOptions.Compiled | WildcardOptions.IgnoreCase);
